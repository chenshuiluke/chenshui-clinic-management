...
describe("Security - Organization Verification and CORS/Headers", () => {
  ...
  beforeEach(async () => {
    app = getApp();
    db = getDb(); 
    // Create a central user and token to authorize organization creation
    const user = await createTestUser(db, {
      email: `central-admin-${Date.now()}@test.com`,
    });
    ...
    authToken = jwtService.generateAccessToken(payload);

    // Use unique org name to avoid conflicts
    const uniqueOrgName = `SecurityTestOrg-cors-${Date.now()}`;

    // Create test organization via API to ensure its database is also created
    testOrg = await createTestOrganizationWithDb(app, authToken, {
      name: uniqueOrgName,
    });
  });
  ...
  describe("Organization Existence Verification", () => {
    ...
    it("should cache organization existence checks", async () => {
      ...
      expect(time2).to.be.lessThan(time1 + 20); // Allow some buffer
    }).timeout(15000);
    ...
    it("should handle URL-encoded organization names", async () => {
      ...
      const spacedOrg = await createTestOrganizationWithDb(app, authToken, {
        name: "Test Hospital",
      });

      const res = await request(app).post("/Test%20Hospital/auth/login").send({
        email: "dummy@example.com",
        password: "password",
      });

      expect(res.status).to.not.equal(404);
      // Cleanup is handled by trackOrganization in the fixture
    }).timeout(15000);
      ...
      responses.forEach((res) => {
        if (res.status === 401) {
          ...
          expect(res.body.error).to.be.oneOf([
            ...
            "Authentication token required", // Added this possibility
          ]);
