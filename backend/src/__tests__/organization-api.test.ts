...
describe("Organization API", () => {
  ...
  describe("GET /organization", () => {
    it("should return an empty array when no organizations exist", async () => {
      ...
      expect(response.body).to.be.an("array", `Expected response body to be an array but got ${typeof response.body}. Body: ${JSON.stringify(response.body)}`);
      expect(response.body).to.have.lengthOf(0, `Expected empty array but got ${response.body.length} organizations: ${JSON.stringify(response.body)}`);
    });
    ...
    it("should return organizations in consistent order", async () => {
      ...
      expect(response1.body).to.deep.equal(response2.body, `Organizations returned in different order. First call: ${JSON.stringify(response1.body)}, Second call: ${JSON.stringify(response2.body)}`);
    });
  });
  ...
  describe("POST /organization", () => {
    ...
    it("should handle duplicate organization names", async () => {
      ...
      expect([400, 409, 500]).to.include(response.status, `Expected status 400, 409, or 500 for duplicate name "${orgName}" but got ${response.status}. Response: ${JSON.stringify(response.body)}`);
    });
  });
  ...
  describe("Organization Data Integrity", () => {
    ...
    it("should set createdAt timestamp", async () => {
      ...
      expect(createdAt.getTime()).to.be.at.least(beforeCreate.getTime(), `Expected createdAt ${createdAt.toISOString()} to be at or after ${beforeCreate.toISOString()}. Response: ${JSON.stringify(response.body)}`);
      expect(createdAt.getTime()).to.be.at.most(afterCreate.getTime(), `Expected createdAt ${createdAt.toISOString()} to be at or before ${afterCreate.toISOString()}. Response: ${JSON.stringify(response.body)}`);
    });
    ...
    it("should set updatedAt timestamp", async () => {
      ...
      expect(updatedAt.getTime()).to.be.at.least(beforeCreate.getTime(), `Expected updatedAt ${updatedAt.toISOString()} to be at or after ${beforeCreate.toISOString()}. Response: ${JSON.stringify(response.body)}`);
      expect(updatedAt.getTime()).to.be.at.most(afterCreate.getTime(), `Expected updatedAt ${updatedAt.toISOString()} to be at or before ${afterCreate.toISOString()}. Response: ${JSON.stringify(response.body)}`);
    });
    ...
    it("should increment organization IDs", async () => {
      ...
      expect(response2.body.id).to.be.greaterThan(response1.body.id, `Expected second organization id ${response2.body.id} to be greater than first organization id ${response1.body.id}. First org: ${JSON.stringify(response1.body)}, Second org: ${JSON.stringify(response2.body)}`);
    });
  });
});
